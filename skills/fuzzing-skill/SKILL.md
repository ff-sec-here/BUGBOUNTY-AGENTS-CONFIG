...
name: fuzzing
description: >
  Expert guidance for ffuf web fuzzing during penetration testing, including authenticated
  fuzzing with raw requests, auto-calibration, and result analysis. Use when the user wants
  to fuzz directories, files, parameters, vhosts, or subdomains. Covers wordlists, ffuf flags,
  filter strategies, and importing results into BB-Mapper.
---
...
## Helper Script
A Python helper script is available at `ffuf_helper.py` in this skill directory:

```bash
# Analyze ffuf JSON results for anomalies and interesting findings
python3 ~/.kiro/skills/fuzzing-skill/ffuf_helper.py analyze results.json

# Create authenticated request template
python3 ~/.kiro/skills/fuzzing-skill/ffuf_helper.py create-req -o req.txt -m POST \
    -u "https://api.target.com/users" -H "Authorization: Bearer TOKEN" -d '{"action":"FUZZ"}'

# Generate IDOR testing wordlist (number range)
python3 ~/.kiro/skills/fuzzing-skill/ffuf_helper.py wordlist -o ids.txt -t numbers -s 1 -e 10000
```

## Importing Results into BB-Mapper

After fuzzing, always import the JSON output:
```
POST /targets/{targetId}/fuzzing/import-ffuf
Body: <ffuf JSON file contents>
```

Then review results:
```
GET /targets/{targetId}/fuzzing/all-results
GET /subdomains/{subdomainId}/fuzzing
```

## Notes for Claude
When helping users with ffuf:
1. **ALWAYS include `-ac` in every command** - mandatory for productive pentesting
2. For authenticated fuzzing: suggest `req.txt` with `--request` flag
3. Always recommend saving output to JSON: `-o results.json -of json`
4. Suggest rate limiting (`-rate`) for production targets
5. When analyzing results: focus on anomalies — different status codes, response sizes, timing
6. Flag interesting endpoints: admin, api, backup, config, .git, etc.
7. Suggest `ffuf_helper.py analyze` for result analysis
